...
class Lcd(Frame):
    ...
    def setup(self):
        ...
        if (SHOW_BUTTONS):
            ...
            self._bpause = tkinter.Button(self, bg="purple", fg="white", font=("Courier New", 18), text="Pause", anchor=CENTER, command=self.pause)
            self._bpause.grid(row=6, column=0, pady=40)
            # the quit button
            self._bquit = tkinter.Button(self, bg="purple", fg="white", font=("Courier New", 18), text="Quit", anchor=CENTER, command=self.quit)
            self._bquit.grid(row=6, column=2, pady=40)
            # the reset button
            self._breset = tkinter.Button(self, bg="purple", fg="white", font=("Courier New", 18), text="Reset", anchor=CENTER, command=self.reset)
            self._breset.grid(row=6, column=1, pady=40)
...
class Timer(PhaseThread):
    def __init__(self, component, initial_value, toggles, name="Timer"):
    ...
    def pause(self):
        global toggle_target
        # toggle the paused state
        self._paused = not self._paused
        # blink the 7-segment display when paused
        self._component.blink_rate = (2 if self._paused else 0)
        value1 = timer_value_button.split(":")
        value2 = "".join(value1)
        sum = 0
        for c in value2:
            sum += int(c)

        self._toggles._target = sum

    # returns the timer as a string (mm:ss)
    def __str__(self):
        global timer_value_button
        timer_value_button = f"{self._min}:{self._sec}"
        return timer_value_button
...
class Wires(PhaseThread):
    ...
    def _get_int_value(self):
        ...
        # bit_values = [ "1", "1", "0", "0"]
        int_values = "".join(bit_values)
        #int_values = "1100"
        int_value = int(int_values, 2)
        # int_value = 12
        return int_value
    ...
    def _check_wrong_state(self):
        ...
        #F otherwise
        #get index od toggled switch using self._value and self._prev_value
