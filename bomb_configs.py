...
def get_random_number_from_string(input_string):
    global serial
    ...
    # Select a random number from the list of integers
    random_number = random.choice(int_numbers)
...
def genKeypadCombination():
    # encrypts a keyword using the "jump over 5" rule
    def encrypt(keyword):
        cipher = ""
        char_to_key = {
            "A": "8", "B": "8", "C": "8",
    ...
    words = ["BANDIT", "BUCKLE", "CANOPY", "DEBATE", "FIERCE", "GIFTED", "IMPACT", "LONELY", "MIGHTY", "NATURE",
             "REBORN", "RECALL", "SYSTEM", "TAKING", "WIDELY"]

    # pick a keyword and matching passphrase
    word = choice(words)
    # encrypt the passphrase and get its combination
    keypad_target = encrypt(word)

    return word, keypad_target
    
    print("Keyword: ", word)
    print("Encrypted Keyword: ", keypad_target)
...
word, keypad_target = genKeypadCombination()
